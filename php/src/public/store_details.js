...
function loadStore(){
    ...
    xhr.open("GET", `api/get_store_info.php?store_id=${storeId}`, true);
    xhr.onload = function() {
        if (xhr.status >= 200 && xhr.status < 400) {
            ...
            container.innerHTML = `
                <img src="${store.store_logo_path}" alt="product image" width="150"  onerror="this.onerror=null; this.src='/public/uploads/ui/placeholder.png'">
                <div>
            ...
            console.error("Gagal memuat info toko");
        }
    };
...
function loadProducts(){
    ...
    const xhr = new XMLHttpRequest();
    xhr.open("GET", "api/get_store_details_info.php?" + params.toString(), true);
    ...
    xhr.onload = function() {
        if (xhr.status >= 200 && xhr.status < 400) {
            ...
            if(catInput){
                ...
                fieldset.innerHTML = "<legend>Kategori</legend>";
...
function submitFilterForm(){
    ...
    formParams.set('store_id', currentParams.get('store_id'));

    const query = formParams.toString();
